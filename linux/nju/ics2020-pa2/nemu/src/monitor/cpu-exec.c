...
void monitor_statistic() {
  Log("total guest instructions = %"PRId64, g_nr_guest_instr);
  Log("host time spent = %"PRId64" ms", g_timer);
  if (g_timer > 0) Log("simulation frequency = %"PRId64" instr/s", g_nr_guest_instr * 1000 / g_timer);
